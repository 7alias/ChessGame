public class Queen extends ChessPiece {
    ...
    public boolean canMoveToPosition(ChessBoard chessBoard, int line, int column, int toLine, int toColumn) {
        // moved like bishop and coordinates are correct
        if (line != toLine && column != toColumn
            ...
            if ((column == getMin(column, toColumn)
                ...
                for (int i = 1; i < toC - fromC; i++) {
                    ...
                    } else if (!chessBoard.board[fromL - i][fromC + i].color
                            .equals(this.color)
                            && fromL - i == toLine) {
                        positions[i - 1] = new int[]{fromL - i, fromC + i};
                    } else {
                ...
                for (int i = 1; i < toC - fromC; i++) {
                    if (chessBoard.board[fromL + i][fromC + i] == null) {
                        positions[i - 1] = new int[]{fromL + i, fromC + i};
                            ...
                            !chessBoard.board[fromL + i][fromC + i]
        ...
        } else if (checkPos(line)
                && checkPos(column)
                && checkPos(toLine)
                && checkPos(toColumn)) {
            // if we move like rook and coordinates are correct
            if (column == toColumn) {
                // from line to line
                for (int i = getMin(line, toLine); i < getMax(line, toLine); i++) {
                    if (chessBoard.board[i][column] != null) {
                        ...
                        else if (chessBoard.board[i][column]
                                .getColor()
                                .equals(this.color)
                                && i == toLine)
                            return false;
                ...
                if (chessBoard.board[toLine][column] != null) {
                    if (chessBoard.board[toLine][column]
                            .getColor()
                            .equals(this.color)
                ...
                for (int i = getMin(toColumn, column); i < getMax(column, toColumn); i++) {
                    if (chessBoard.board[line][i]
                            != null) {
                        if (chessBoard.board[line][i]
                                == this && i == getMax(column, toColumn)) return false;
                        else if (chessBoard.board[line][i]
                                .getColor().equals(this.color)
                                && i == toColumn)
                            return false;
                        else if (!chessBoard.board[line][i]
                ...
                if (chessBoard.board[toLine][toColumn] != null) {
                    if (chessBoard.board[toLine][toColumn]
                            .getColor()
                            .equals(this.color)
                            && chessBoard.board[toLine][toColumn] != this)
                        return false;
                    else
                        return !chessBoard.board[toLine][toColumn]
                                .getColor().equals(this.color)
